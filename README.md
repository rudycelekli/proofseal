...
- **Non-deterministic test suites can't be harness claims.** Harness claims require seeded, deterministic numeric output. Parallel test runners, wall-clock timings, and unordered output will produce permanent drift or regression noise. Common textual noise (timestamps, UUIDs, abs paths, ANSI escapes, JSON key order) can be masked with opt-in `--normalize` flags (`proofseal harness diagnose <name>` will identify which apply). The normalizers harden the existing **numeric** harness — they do not make arbitrary text or JSON output sealable; the hash is still over the quantized numeric vector. `mask-paths` preserves the basename, so a nondeterministic basename like `run-NNNNNN.json` still leaks — use `exclude` or change the harness for that case. Verify reads the normalizer set off the **signed** manifest, not local config, so editing `proofseal.json` after sealing cannot change what verify masks.
